...
#
# run-dpu-veth-nat-proof.sh
#
# One-shot host-side harness for the PF1 veth-based Linux NAT experiment.

set -euo pipefail

SCRIPT_NAME="dpu-veth-proof"
SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
source "$SCRIPT_DIR/common.sh"

usage() {
    cat <<EOF
Usage: sudo ./scripts/run-dpu-veth-nat-proof.sh [OPTIONS]

Runs the PF1 veth-based NAT proof end-to-end from the host.
...
log "Step 1/4: configure DPU for veth-based NAT"
DPU_SSH_KEEP_MASTER=true "$SCRIPT_DIR/test-dpu-veth-nat.sh" --dpu-host "$DPU_HOST" --dpu-user "$DPU_USER"

log "Step 2/4: start remote captures on DPU"
REMOTE_DIR=$(ssh_dpu "mktemp -d /tmp/openshell-veth-proof.XXXXXX")
...
timeout ${CAPTURE_TIMEOUT}s tcpdump -nn -i hostnat0 \"host 10.99.2.2 or host 10.185.99.182 or host ${DEST_IP}\" > ${REMOTE_DIR}/hostnat0.txt 2>&1 &
timeout ${CAPTURE_TIMEOUT}s tcpdump -nn -i enp3s0f1s0 \"host 10.99.2.2 or host 10.185.99.182 or host ${DEST_IP}\" > ${REMOTE_DIR}/enp3s0f1s0.txt 2>&1 &
...
echo
echo "=== DPU tcpdump: hostnat0 ==="
ssh_dpu "cat ${REMOTE_DIR}/hostnat0.txt 2>/dev/null || true"
echo
echo "=== DPU tcpdump: enp3s0f1s0 ==="
ssh_dpu "cat ${REMOTE_DIR}/enp3s0f1s0.txt 2>/dev/null || true"
echo
echo "=== DPU tcpdump: p1 ==="
ssh_dpu "cat ${REMOTE_DIR}/p1.txt 2>/dev/null || true"
echo
echo "=== DPU iptables FORWARD ==="
ssh_dpu "sudo iptables -L FORWARD -n -v"
echo
echo "=== DPU iptables raw PREROUTING ==="
ssh_dpu "sudo iptables -t raw -L PREROUTING -n -v"
echo
echo "=== DPU nft POSTROUTING ==="
ssh_dpu "sudo nft list chain ip nat POSTROUTING || true"
echo
echo "=== DPU ctdebug ==="
ssh_dpu "sudo nft list chain inet ctdebug forward || true"
echo

...
# Match the full command line without matching this wrapper's own ssh command.
pkill -f '[o]penshell-dpu-proxy' 2>/dev/null || true
pkill -f "[o]pa run --server --addr $OPA_ADDR" 2>/dev/null || true

for _ in $(seq 1 10); do
    if ! ss -tln 2>/dev/null | grep -q ':${PROXY_LISTEN##*:}\b'; then
        break
    fi
    sleep 1
done
